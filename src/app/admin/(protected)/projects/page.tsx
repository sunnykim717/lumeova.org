  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-[24px] text-navy">사업 관리</h1>
        <Link
            ...
            {(projects ?? []).length === 0 ? (
              ...
              (projects ?? []).map((p) => (
                <tr key={p.id} className="border-b border-navy/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-muted">{STATUS_LABEL[p.status]}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-3">
                      <Link href={`/admin/projects/${p.id}/edit`} className="text-[12px] text-navy underline underline-offset-4">
                        수정
                      </Link>
                      <DeleteButton id={p.id} action={deleteProject} confirmLabel="이 사업을 삭제할까요?" />
                    </div>
                  </td>
                </tr>
              ))
